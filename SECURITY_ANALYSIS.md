...
### 🟡 Finding #4: Swift Secret Key Exposure in Metadata

**Severity:** MEDIUM | **Confidence:** HIGH | **CWE:** CWE-200, CWE-522

**Impact:**  
Container and account metadata routes return raw TempURL signing keys and sync keys to browser clients. These keys enable unauthenticated object access and cross-account synchronization.

**Exposed Keys:**

- `x-container-sync-key` - Container synchronization secret
- `x-container-meta-temp-url-key` - Primary TempURL signing key
- `x-container-meta-temp-url-key-2` - Secondary TempURL signing key (rotation)
- `x-account-meta-temp-url-key` - Account-level TempURL key
- `x-account-meta-temp-url-key-2` - Account-level secondary key

**Attack Vector:**

```typescript
// Request container metadata
GET /trpc/swift.getContainerMetadata
{ "container": "my-container" }

// Response includes secrets:
{
  "syncKey": "super-secret-sync-key-123",
  "tempUrlKey": "signing-key-abc",
  "tempUrlKey2": "signing-key-xyz"
}

// Attacker can now:
// 1. Generate valid TempURLs for any object
// 2. Enable unauthorized sync to attacker-controlled Swift
```

**Vulnerable Code:**

```typescript
// packages/aurora/src/server/Storage/helpers/swiftHelpers.ts:207-219
const syncKey = headers.get("x-container-sync-key")
if (syncKey) {
  containerInfo.syncKey = syncKey // ❌ Secret returned to browser
}

const tempUrlKey = headers.get("x-container-meta-temp-url-key")
if (tempUrlKey) {
  containerInfo.tempUrlKey = tempUrlKey // ❌ Secret returned to browser
}
```
...
```typescript
// Redact secrets from metadata responses
export function parseContainerInfo(headers: Headers): ContainerInfo {
  ...
  return {
    ...
    // ❌ Do not return: syncKey, tempUrlKey, tempUrlKey2
  }
}
```

**Priority:** P1 - Address in current sprint

---

### 🟡 Finding #5: Overbroad Swift TempURL Generation

**Severity:** MEDIUM | **Confidence:** HIGH | **CWE:** CWE-284, CWE-639

**Impact:**  
Users can mint long-lived unauthenticated URLs supporting mutating HTTP methods (`PUT`, `POST`, `DELETE`) with unlimited lifetimes.

**Current Issues:**

1. ✅ Allows `GET`, `PUT`, `POST`, `DELETE` methods
2. ✅ No upper bound on lifetime (`expiresIn >= 1` second = min 1s, max unlimited)
3. ✅ No BFF-side scope restrictions on objects
4. ✅ No audit trail for URL generation

**Attack Vector:**

```typescript
// Generate 10-year write URL
POST /trpc/swift.generateTempUrl
{
...
// Response: valid unauthenticated DELETE URL
{
  "url": "https://swift.example.com/v1/AUTH_abc/sensitive-data/config.json?temp_url_sig=...&temp_url_expires=1899648000"
}

// Attacker shares URL publicly → anyone can delete the object
```

**Vulnerable Code:**

```typescript
// packages/aurora/src/server/Storage/types/swift.ts:363-365
export const generateTempUrlInputSchema = baseObjectInputSchema.extend({
  method: z.enum(["GET", "PUT", "POST", "DELETE"]), // ❌ Mutating methods allowed
  expiresIn: z.number().min(1), // ❌ No upper bound
})
```
...
```typescript
// 1. Cap lifetime
const MAX_TEMP_URL_LIFETIME = 3600 // 1 hour
export const generateTempUrlInputSchema = baseObjectInputSchema.extend({
  method: z.enum(["GET"]), // Default to read-only
  expiresIn: z.number().min(1).max(MAX_TEMP_URL_LIFETIME),
})

// 2. Require explicit privilege for mutating methods
if (input.method !== "GET") {
  const hasWritePrivilege = await checkUserPrivilege(ctx, "swift:tempurl:write")
  if (!hasWritePrivilege) {
    throw new TRPCError({ code: "FORBIDDEN", message: "Write TempURLs require elevated privilege" })
  }
}

// 3. Add audit logging
logger.info("TempURL generated", {
  userId: ctx.user.id,
  projectId: ctx.scopedProjectId,
