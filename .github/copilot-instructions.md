...
- **No conflicting `Message` inside a modal.** A confirmation or destructive modal should **not** carry an extra `Message` in its body. If a `Message` is genuinely needed, its variant must match the primary button: `info` ↔ blue `primary`, `danger` ↔ `primary-danger`. Never mix conflicting semantic colors in one modal.
