...
const createCaller = createCallerFactory(router(cloudProfilesRouter(k8sClientMock)))
const caller = createCaller({})

describe("cloudProfilesRouter", () => {
  let router: ReturnType<typeof cloudProfilesRouter>

  beforeEach(() => {
    vi.clearAllMocks()
    router = cloudProfilesRouter(k8sClientMock)
  })
  ...
  describe("getCloudProfiles", () => {
    ...
    it("should successfully fetch and convert cloud profiles", async () => {
      // Arrange
      vi.mocked(k8sClientMock.get).mockResolvedValue(validApiResponseMock)

      // Act
      const result = await caller.getCloudProfiles()

      // Assert
      expect(k8sClientMock.get).toHaveBeenCalledWith("apis/core.gardener.cloud/v1beta1/cloudprofiles")
