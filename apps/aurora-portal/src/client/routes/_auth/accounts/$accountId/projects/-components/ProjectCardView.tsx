...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <Box
      className="group border-theme-background-lvl-3 bg-theme-background-lvl-1 hover:border-theme-accent/30 relative flex min-h-[140px] cursor-pointer flex-col overflow-hidden rounded-xl border p-6 shadow-sm transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-xl"
    ...
    >
      <div className="flex flex-1 flex-col">
        ...
        {/* Description Section */}
        {project.description && (
          <p className="text-theme-default/90 line-clamp-3 leading-relaxed">{project.description}</p>
        )}
        {!project.description && <p className="text-theme-light italic">No description</p>}
      </div>
