...
export const CopyObjectModal = ({ isOpen, object, onClose, onSuccess, onError }: CopyObjectModalProps) => {
  ...
  // These are merged into the fetched object list so newly created folders
  // appear immediately without waiting for a server refetch.
  ...
  // Inject locally created folders that are at the current prefix level
  // and not yet reflected in the server response
  const localFolderRows: BrowserRow[] = containerLocalFolders
    .filter((fp) => {
      // fp is a full path like "prefix/newfolder/"
      // show it only if its parent prefix matches currentPrefix
      const withoutPrefix = fp.startsWith(currentPrefix) ? fp.slice(currentPrefix.length) : null
      if (!withoutPrefix) return false
      // At this level means only one segment left (e.g. "newfolder/")
  ...
  const validateNewFolderName = useCallback(
    (name: string): boolean => {
      ...
      // Check duplicate against existing rows
  ...
  const handleCopy = () => {
    ...
    // destination format: "/targetContainer/prefix/filename" (plain text)
                    ...
                    <Stack direction="horizontal" gap="2" alignment="center">
                      ...
                      <TextInput
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                          ...
                          // Clear error on change but don't re-validate on every keystroke
