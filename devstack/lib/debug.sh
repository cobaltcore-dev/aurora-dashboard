...
show_help() {
    ...
    echo -e "  ./devstack debug <subcommand> [options]"
    echo -e ""
    ...
    echo -e "  ./devstack debug logs nova"
    echo -e "  ./devstack debug status"
    echo -e "  ./devstack debug ovs"
    echo -e "  ./devstack debug api keystone"
    echo -e "  ./devstack debug network"
    echo -e "  ./devstack debug compute"
    echo -e "  ./devstack debug all"
    echo -e ""
...
case "$SUBCOMMAND" in
    logs)
        ...
        if [ -z "$SERVICE" ]; then
            ...
            info "Usage: ./devstack debug logs <service>"
            echo ""
    ...
    api)
        ...
        if [ -z "$SERVICE" ]; then
            error "Service name required"
            info "Usage: ./devstack debug api <service>"
            echo ""
            info "Available services (currently running):"
            services=$(get_devstack_services)
            if [ -n "$services" ]; then
