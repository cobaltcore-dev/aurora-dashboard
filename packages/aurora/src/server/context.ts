...
): Promise<AuroraPortalContext> {
  // Normalise identity endpoint to always end with a single slash
  const normalizedEndpoint = config.identityEndpoint.endsWith("/")
    ? config.identityEndpoint
    : `${config.identityEndpoint}/`

  // Proxy is ONLY enabled in development mode for security reasons
  const proxyConfig = process.env.NODE_ENV !== "production" && config.proxyUrl ? { uri: config.proxyUrl } : undefined

  if (process.env.NODE_ENV === "production" && config.proxyUrl) {
    console.warn("⚠️ [context] proxyUrl is set but ignored in production mode for security reasons")
  } else if (proxyConfig) {
    const parsed = new URL(proxyConfig.uri)
    parsed.password = parsed.password ? "***" : ""
    console.log("ℹ️ [context] Proxy configured:", parsed.toString())
  }

  if (process.env.NODE_ENV !== "production") {
    console.log("ℹ️ [endpoint] Identity endpoint:", normalizedEndpoint)
    console.log("ℹ️ [context] Default endpoint interface:", config.defaultEndpointInterface)
  }
