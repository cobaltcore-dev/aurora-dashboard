...
describe("corsRouter", () => {
  ...
  describe("set", () => {
    ...
    it("should accept large MaxAgeSeconds values (no maximum per AWS S3 spec)", async () => {
      mockSend.mockResolvedValueOnce({})
          ...
          CORSRules: [
            {
              AllowedMethods: ["GET"],
              AllowedOrigins: ["*"],
              MaxAgeSeconds: 604800, // 7 days - common CDN cache time
            },
          ],
        },
      })
    ...
    it("should enforce rate limiting (10 changes per minute per bucket)", async () => {
      ...
      const rateLimitBucket = "cors-rate-limit-test-bucket-unique"
    ...
    it("schedules per-key cleanup that fires when the window closes", async () => {
      ...
      try {
        mockSend.mockResolvedValue({})
        const bucket = "cors-cleanup-bucket-unique"
        await caller.set({
          project_id: TEST_PROJECT_ID,
          bucketName: bucket,
          corsConfiguration: {
