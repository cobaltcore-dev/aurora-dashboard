...
export const SideNavBar = ({ projectId, projectName, availableServices }: SideNavBarProps) => {
  ...
  // When navigating into a section, persist it as open so our state stays in
  // sync with Juno's internal state when we later navigate away.
  if (activeSection && activeSection in openSections && !openSections[activeSection as keyof typeof openSections]) {
