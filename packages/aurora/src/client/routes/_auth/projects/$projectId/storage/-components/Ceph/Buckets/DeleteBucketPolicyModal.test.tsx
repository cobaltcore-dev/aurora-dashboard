...
vi.mock("@/client/trpcClient", () => ({
  trpcReact: {
    useUtils: () => ({
      storage: {
        ceph: {
          bucketPolicy: { get: { invalidate: mockInvalidate } },
        },
      },
    }),
    storage: {
      ceph: {
        bucketPolicy: {
          get: {
            useQuery: () => ({
              data: { policy: { Version: "2012-10-17" }, policyText: "{}" },
              isLoading: false,
              error: null,
            }),
          },
...
describe("DeleteBucketPolicyModal - Analytics tracking", () => {
  beforeEach(async () => {
    vi.clearAllMocks()
  ...
  test("tracks .open event when modal opens", async () => {
    ...
    await waitFor(() => {
      expect(mockOnTrackEvent).toHaveBeenCalledWith({
        source: "user-action",
        action: "storage.ceph.bucket.policy.delete.open",
        metadata: {
          accessed: true,
        },
      })
  ...
  test("tracks .close event when user cancels without deleting", async () => {
    ...
    expect(mockOnTrackEvent).toHaveBeenCalledWith({
      source: "user-action",
      action: "storage.ceph.bucket.policy.delete.close",
      metadata: {
        cancelled: true,
      },
    })
