...
describe("EnableVersioningModal - Analytics tracking", () => {
  beforeEach(async () => {
    vi.clearAllMocks()
  ...
  test("tracks .open event when modal opens", async () => {
    ...
    await waitFor(() => {
      expect(mockOnTrackEvent).toHaveBeenCalledWith({
        source: "modal",
        action: "storage.ceph.bucket.versioning.enable.open",
      })
    })
  ...
  test("tracks .close event when user cancels without enabling", async () => {
    ...
    expect(mockOnTrackEvent).toHaveBeenCalledWith({
      source: "modal",
      action: "storage.ceph.bucket.versioning.enable.close",
    })
