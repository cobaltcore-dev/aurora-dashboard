// Module-scope registry of in-flight Swift object downloads.
//
...
// navigation, spinner swaps, and even leaving the container entirely — which is
// what the download-started toast already promises the user.
//
...
// Keyed by transferKey() — container-scoped so the same object name in two
// containers can't collide.
...
// repeated id as an update, not a new notification. Distinct from the Ceph
// store's id so the two backends' persistent toasts never dismiss each other.
const DOWNLOAD_TOAST_ID = "swift-object-download"
...
// BFF actually returns, not from the filename.
//
...
export function startObjectDownload(opts: {
  ...
  const { kind, projectId, container, objectKey, filename, account, onError } = opts
  const key = transferKey(container, objectKey)

  // Ignore a second start for a row that's already transferring.
  if (transfers.has(key)) return

  const downloadId = `${container}:${objectKey}:${crypto.randomUUID()}`
...
// from Swift. The worker isn't terminated right away — it unwinds its own abort
// and is dropped once idle, avoiding a mid-stream hard kill; a grace-period timer
...
export function cancelObjectDownload(container: string, objectKey: string): ActiveTransfer | undefined {
  const key = transferKey(container, objectKey)
