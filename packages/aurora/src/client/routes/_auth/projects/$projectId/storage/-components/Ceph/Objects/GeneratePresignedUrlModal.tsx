import { useEffect, useRef, useState, type ChangeEvent } from "react"
import { Trans, useLingui } from "@lingui/react/macro"
import { i18n } from "@lingui/core"
import { trpcReact } from "@/client/trpcClient"
import { useProjectId } from "@/client/hooks/useProjectId"
import { S3_PRESIGN_MAX_EXPIRY_SECONDS } from "@/server/Storage/constants"
import { Modal, Stack, Spinner, TextInput, Icon, Select, SelectOption } from "@cloudoperators/juno-ui-components"

// ── Expiry presets ────────────────────────────────────────────────────────────

interface ExpiryPreset {
  labelKey: string
  seconds: number
}

const CUSTOM_VALUE = "custom"

// S3 SigV4 pre-signed URLs are valid for at most 7 days. Reuse the BFF cap
// (S3_PRESIGN_MAX_EXPIRY_SECONDS) so the UI and schema can't drift, and reject
// the custom input inline instead of round-tripping to a 400.
const MAX_EXPIRY_SECONDS = S3_PRESIGN_MAX_EXPIRY_SECONDS
const MAX_EXPIRY_MINUTES = MAX_EXPIRY_SECONDS / 60

// ── Props ─────────────────────────────────────────────────────────────────────

interface GeneratePresignedUrlModalProps {
  isOpen: boolean
  bucketName: string
  // Full object key, or null when no row is targeted (modal closed).
  objectKey: string | null
  onClose: () => void
  // Called with the full object key after a successful clipboard copy so the
  // caller can raise a confirmation toast.
  onCopySuccess?: (objectKey: string) => void
}

// ── Component ─────────────────────────────────────────────────────────────────

export const GeneratePresignedUrlModal = ({
  isOpen,
  bucketName,
  objectKey,
  onClose,
  onCopySuccess,
}: GeneratePresignedUrlModalProps) => {
  const { t } = useLingui()
  const projectId = useProjectId()

  // Defined inside the component so t`` runs in the correct Lingui context,
  // ensuring labels are extracted and translated at render time.
  const EXPIRY_PRESETS: ExpiryPreset[] = [
    { labelKey: t`1 hour`, seconds: 3600 },
    { labelKey: t`24 hours`, seconds: 86400 },
    { labelKey: t`7 days`, seconds: 604800 },
  ]

  // Selected preset key (seconds as string) or "custom"
  const [selectedPreset, setSelectedPreset] = useState<string>(String(EXPIRY_PRESETS[1].seconds))
  // Custom duration in minutes (only active when selectedPreset === "custom")
  const [customMinutes, setCustomMinutes] = useState("")
  const [customMinutesError, setCustomMinutesError] = useState<string | null>(null)
  const [copied, setCopied] = useState(false)

  // Generated URL state
  const [presignedUrl, setPresignedUrl] = useState<string | null>(null)
  const [expiresAt, setExpiresAt] = useState<number | null>(null)
  const [generalError, setGeneralError] = useState<string | null>(null)
  const [copyError, setCopyError] = useState<string | null>(null)

  const objectKeyRef = useRef("")
  const copyTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  // Incremented on every generate call and on modal close/reopen.
  // Callbacks check their captured requestId against the ref to discard
  // stale in-flight responses that arrive after the modal was closed or
  // a new request was started.
  const requestIdRef = useRef(0)

  const handleGenerateSuccess = (data: { url: string; expiresAt: number }) => {
    setPresignedUrl(data.url)
    setExpiresAt(data.expiresAt)
    setGeneralError(null)
  }

  const handleGenerateError = (error: { message: string }) => {
    setGeneralError(error.message)
  }

  const generateMutation = trpcReact.storage.ceph.objects.generatePresignedUrl.useMutation()

  useEffect(() => {
    if (!isOpen) {
      requestIdRef.current += 1
      if (copyTimeoutRef.current) {
        clearTimeout(copyTimeoutRef.current)
        copyTimeoutRef.current = null
      }
      generateMutation.reset()
      setPresignedUrl(null)
      setExpiresAt(null)
      setSelectedPreset(String(EXPIRY_PRESETS[1].seconds))
      setCustomMinutes("")
      setCustomMinutesError(null)
      setCopied(false)
      setGeneralError(null)
      setCopyError(null)
    }
    return () => {
      if (copyTimeoutRef.current) clearTimeout(copyTimeoutRef.current)
    }
  }, [isOpen])

  // Derive a human-friendly display name from the full key (basename), matching
  // how the object toasts label objects.
  const displayName = objectKey ? (objectKey.split("/").filter(Boolean).pop() ?? objectKey) : ""

  // Parses customMinutes as a strict positive integer.
  // Number() + Number.isInteger() rejects non-integer inputs such as "1.5" → 1.5
  // and "10abc" → NaN with parseInt, but null here.
  // Numeric formats accepted by Number() (for example, "1e3") are still allowed
  // if they evaluate to a positive integer.
  const parseCustomMinutes = (): number | null => {
    if (!customMinutes.trim()) return null
    const mins = Number(customMinutes)
    return Number.isInteger(mins) && mins > 0 ? mins : null
  }

  const resolveExpiresIn = (): number | null => {
    if (selectedPreset === CUSTOM_VALUE) {
      const mins = parseCustomMinutes()
      if (mins === null) return null
      return mins * 60
    }
    return parseInt(selectedPreset, 10)
  }

  const handleGenerate = () => {
    if (!objectKey) return
    if (selectedPreset === CUSTOM_VALUE) {
      const mins = parseCustomMinutes()
      if (mins === null) {
        setCustomMinutesError(t`Please enter a valid number of minutes greater than 0`)
        return
      }
      if (mins > MAX_EXPIRY_MINUTES) {
        setCustomMinutesError(t`A pre-signed URL can be valid for at most 7 days (${MAX_EXPIRY_MINUTES} minutes)`)
        return
      }
      setCustomMinutesError(null)
    }
    const expiresIn = resolveExpiresIn()
    if (expiresIn === null) return
    objectKeyRef.current = objectKey
    setPresignedUrl(null)
    setExpiresAt(null)
    setCopied(false)
    const requestId = ++requestIdRef.current
    generateMutation.mutate(
      {
        project_id: projectId,
        containerName: bucketName,
        objectKey,
        expiresIn,
      },
      {
        onSuccess: (data) => {
          if (requestId !== requestIdRef.current) return
          handleGenerateSuccess(data)
        },
        onError: (error) => {
          if (requestId !== requestIdRef.current) return
          handleGenerateError(error)
        },
      }
    )
  }

  const handleCopy = () => {
    if (!presignedUrl) return
    setCopyError(null)
    navigator.clipboard
      .writeText(presignedUrl)
      .then(() => {
        setCopied(true)
        onCopySuccess?.(objectKeyRef.current)
        if (copyTimeoutRef.current) clearTimeout(copyTimeoutRef.current)
        copyTimeoutRef.current = setTimeout(() => {
          copyTimeoutRef.current = null
          setCopied(false)
        }, 2000)
      })
      .catch(() => {
        setCopyError(t`Failed to copy the pre-signed URL to the clipboard`)
      })
  }

  const handlePresetChange = (value?: string | number | string[]) => {
    const defaultPreset = String(EXPIRY_PRESETS[1].seconds)
    const candidate =
      typeof value === "string"
        ? value
        : typeof value === "number"
          ? String(value)
          : Array.isArray(value) && typeof value[0] === "string"
            ? value[0]
            : ""
    const validPresets = new Set([...EXPIRY_PRESETS.map((p) => String(p.seconds)), CUSTOM_VALUE])
    const nextPreset = candidate && validPresets.has(candidate) ? candidate : defaultPreset
    setSelectedPreset(nextPreset)
    // Clear custom input state when switching away from custom
    if (nextPreset !== CUSTOM_VALUE) {
      setCustomMinutes("")
      setCustomMinutesError(null)
    }
    // Reset generated URL when config changes
    setPresignedUrl(null)
    setExpiresAt(null)
    setCopied(false)
    setGeneralError(null)
    setCopyError(null)
  }

  const handleCustomMinutesChange = (e: ChangeEvent<HTMLInputElement>) => {
    setCustomMinutes(e.target.value)
    if (customMinutesError) setCustomMinutesError(null)
    // Reset generated URL when config changes
    setPresignedUrl(null)
    setExpiresAt(null)
    setCopied(false)
    setGeneralError(null)
    setCopyError(null)
  }

  if (!isOpen || !objectKey) return null

  const isPending = generateMutation.isPending
  const isCustom = selectedPreset === CUSTOM_VALUE

  // Absolute expiry timestamp formatted for the active Lingui locale (not the
  // browser's OS locale) so it matches the rest of the modal's localization.
  const expiresAtFormatted = expiresAt ? new Date(expiresAt * 1000).toLocaleString(i18n.locale) : null
  // Human-readable relative label for the selected preset — used as a plain
  // string inside the "Expires in {label} — at {timestamp}" interpolation.
  const selectedPresetLabel =
    selectedPreset === CUSTOM_VALUE
      ? customMinutes
        ? parseCustomMinutes() === 1
          ? t`1 minute`
          : t`${customMinutes} minutes`
        : null
      : (EXPIRY_PRESETS.find((p) => String(p.seconds) === selectedPreset)?.labelKey ?? null)

  return (
    <Modal
      title={
        <span className="flex max-w-100 items-center gap-1">
          <span className="shrink-0">
            <Trans>Share URL:</Trans>
          </span>
          <span className="truncate" title={displayName}>
            {displayName}
          </span>
        </span>
      }
      open={isOpen}
      onCancel={onClose}
      confirmButtonLabel={isPending ? t`Generating...` : t`Generate URL`}
      onConfirm={handleGenerate}
      cancelButtonLabel={t`Cancel`}
      size="small"
      disableConfirmButton={isPending || (isCustom && (!customMinutes.trim() || !!customMinutesError))}
    >
      <Stack direction="vertical" gap="4">
        <p className="text-theme-default">
          <Trans>
            A pre-signed URL grants time-limited read access to this object without requiring authentication. Anyone
            with the link can download it until it expires.
          </Trans>
        </p>

        {/* General error */}
        {generalError && (
          <p className="text-theme-error" role="alert" aria-live="assertive">
            <Trans>Failed to generate pre-signed URL: {generalError}</Trans>
          </p>
        )}

        {/* Clipboard copy error */}
        {copyError && (
          <p className="text-theme-error" role="alert" aria-live="assertive">
            {copyError}
          </p>
        )}

        {/* Expiry selector */}
        <Select label={t`Expires in`} value={selectedPreset} onChange={handlePresetChange} disabled={isPending}>
          {EXPIRY_PRESETS.map((preset) => (
            <SelectOption key={preset.seconds} value={String(preset.seconds)} label={preset.labelKey} />
          ))}
          <SelectOption value={CUSTOM_VALUE} label={t`Custom`} />
        </Select>

        {/* Custom duration input */}
        {isCustom && (
          <TextInput
            label={t`Custom duration (minutes)`}
            value={customMinutes}
            onChange={handleCustomMinutesChange}
            invalid={!!customMinutesError}
            errortext={customMinutesError || undefined}
            disabled={isPending}
            placeholder="60"
            type="number"
          />
        )}

        {/* Loading state */}
        {isPending && (
          <Stack direction="horizontal" alignment="center" gap="2" className="py-2">
            <Spinner size="small" />
            <Trans>Generating pre-signed URL...</Trans>
          </Stack>
        )}

        {/* Generated URL */}
        {presignedUrl && (
          <Stack direction="vertical" gap="2">
            <div className="relative">
              <TextInput label={t`Pre-signed URL`} value={presignedUrl} readOnly className="pr-10 text-xs" />
              <button
                type="button"
                onClick={handleCopy}
                title={copied ? t`Copied!` : t`Copy URL`}
                aria-label={copied ? t`Copied!` : t`Copy URL`}
                className="text-theme-light hover:text-theme-default absolute top-1/2 right-2 inline-flex items-center transition-colors"
                style={{ transform: "translateY(-50%)" }}
              >
                <Icon icon={copied ? "checkCircle" : "contentCopy"} size="18" />
              </button>
            </div>
            {expiresAtFormatted && selectedPresetLabel && (
              <p className="text-theme-light text-xs">
                <Trans>
                  Expires in {selectedPresetLabel} — at {expiresAtFormatted}
                </Trans>
              </p>
            )}
          </Stack>
        )}
      </Stack>
    </Modal>
  )
}
