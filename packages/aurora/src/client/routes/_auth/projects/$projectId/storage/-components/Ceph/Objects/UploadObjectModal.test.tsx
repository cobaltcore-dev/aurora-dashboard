...
describe("UploadObjectModal", () => {
  ...
  describe("Submission", () => {
    test("calls trpcClient.uploadObject.mutate with correct file and headers", async () => {
      const user = userEvent.setup()
    ...
    test("does not send a Swift account header (Ceph has no account concept)", async () => {
      const user = userEvent.setup()
      renderModal()
      const fileInput = document.querySelector("input[type=file]") as HTMLInputElement
      await user.upload(fileInput, makeFile())
