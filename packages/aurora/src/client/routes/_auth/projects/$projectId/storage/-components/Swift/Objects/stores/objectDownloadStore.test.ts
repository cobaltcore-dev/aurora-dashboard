...
describe("objectDownloadStore (swift)", () => {
  it("spawns a worker and posts a start message with a scoped downloadId", () => {
    ...
    expect(msg).toMatchObject({ type: "start", projectId: "p", container: "b", objectKey: "k.zip" })
  ...
  describe("the start message", () => {
    ...
    it("carries the account when provided (cross-account download)", () => {
      // Swift-specific: admin cross-account downloads pass an account that the
      // worker forwards to the downloadObject mutation.
      const { worker } = start({ account: "AUTH_other" })
      expect(worker.posted[0]).toMatchObject({ account: "AUTH_other" })
    })

    it("omits account entirely for a same-account download", () => {
      const { worker } = start()
      expect(worker.posted[0]).not.toHaveProperty("account")
    })
  })
  ...
  describe("the shared 'Downloading...' toast", () => {
    it("is raised once for the first transfer and not repeated for further ones", () => {
      ...
      expect(toastMock).toHaveBeenCalledWith(
        "Downloading...",
        expect.objectContaining({ id: "swift-object-download", duration: Infinity })
      )
    ...
    it("is dismissed only once the last transfer ends", () => {
      ...
      expect(toastMock.dismiss).toHaveBeenCalledWith("swift-object-download")
    })
