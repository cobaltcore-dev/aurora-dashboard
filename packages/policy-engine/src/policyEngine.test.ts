...
describe("PolicyEngine", () => {
  ...
  describe("debug functionality", () => {
    ...
    it("logs parameter analysis in debug mode", () => {
      ...
      const hasParameterInfo = logCalls.some(
        (call: unknown) => typeof call === "string" && call.includes("Required parameters")
      )
    ...
    it("logs rule evaluation trace", () => {
      ...
      const logCalls = consoleSpy.mock.calls.flat()
      const hasRuleTrace = logCalls.some(
        (call: unknown) => typeof call === "string" && call.includes("Starting policy check")
      )
