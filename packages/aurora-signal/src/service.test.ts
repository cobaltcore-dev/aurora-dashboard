...
import { AuroraSignalSession, Session } from "./session"

describe("service", () => {
  let session: AuroraSignalSession
  beforeAll(() => {
    ...
    session = new Session("http://localhost", {
      userId: "user",
      password: "password",
  ...
  it("should create a service", () => {
    const service = Service("name", session, {})
  ...
  describe("serviceOptions", () => {
    ...
    it("should respond to get with options: interface", async () => {
      const service = Service("service1", session, { region: "region", interfaceName: "public" })
      expect(service.get("path", {})).toBeInstanceOf(Promise)
    })

    it("should throw error for unknown interface", async () => {
      const service = Service("service1", session, { region: "region", interfaceName: "unknown" })
      await expect(service.get("path", {})).rejects.toThrow(
    ...
    it("should overwrite options", async () => {
      const service = Service("service1", session, { region: "unknown", interfaceName: "unknown" })
