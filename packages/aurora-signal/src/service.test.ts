...
describe("service", () => {
  ...
  beforeAll(() => {
    ...
    token = new AuroraSignalToken(TestToken, "token")
  })

  it("should create a service", () => {
    const service = Service("name", token, {})
  ...
  describe("serviceOptions", () => {
    ...
    it("should respond to get with options: interface", async () => {
      const service = Service("service1", token, { region: "region", interfaceName: "public" })
      expect(service.get("path", {})).toBeInstanceOf(Promise)
    })

    it("should throw error for unknown interface", async () => {
      const service = Service("service1", token, { region: "region", interfaceName: "unknown" })
      await expect(service.get("path", {})).rejects.toThrow(
    ...
    it("should overwrite options", async () => {
      const service = Service("service1", token, { region: "unknown", interfaceName: "unknown" })
